window.TRANSLATION_DATA = {
  "en": {
    ...
    "work_opencl_sub": "8.4× Gaussian blur speedup on Android",
    "work_opencl_date": "2023.12",
    "detail_opencl_title": "On-Device GPU Image Processing via OpenCL & Android NDK",
    "detail_opencl_subtitle": "8.4× Gaussian blur speedup through JNI-bridged GPGPU kernels on Android",
    "detail_opencl_role": "Sole developer",
    "detail_opencl_timeline": "2023.09 – 2023.12",
    "detail_opencl_stack": "Android (Java), OpenCL 1.2, C (NDK/JNI), CMake 3.22",
    "detail_opencl_overview": "Built an Android application that captures images via the device camera and applies real-time image processing—Gaussian blur, grayscale conversion, and rotation—by dispatching OpenCL kernels to the mobile GPU through a JNI-bridged native library. The project was the final deliverable for a microprocessor systems lab course and served as a hands-on introduction to GPGPU programming on a resource-constrained embedded platform where publicly available documentation was minimal.",
    "detail_opencl_point1": "Implemented a full GPGPU pipeline in C using OpenCL 1.2: platform/device enumeration, context and command-queue creation, kernel compilation at runtime, buffer I/O (clCreateBuffer / clEnqueueWriteBuffer / clEnqueueReadBuffer), and explicit resource release — entirely from scratch on Android NDK.",
    "detail_opencl_point2": "Achieved an 8.4× end-to-end speedup for Gaussian blur by offloading pixel-parallel computation to the GPU (CPU: 2059 ms → GPU: 246 ms), and additionally implemented Grayscale (81 ms) and Rotate (72 ms) kernels demonstrating sustained GPU throughput across heterogeneous workloads.",
    "detail_opencl_point3": "Designed a CMake + NDK build pipeline that links libGLES_mali.so and jnigraphics, compiles OpenCLDriver.c as a shared library, and exposes four JNI-callable native methods (GaussianBlurBitmap, GaussianBlurGPU, GaussianGreyGPU, GaussianRotateGPU) to the Java activity layer.",
    "detail_opencl_arch_heading": "System Architecture",
    "detail_opencl_arch_body": "Android Java Activity (UI + Button events) calls JNI native methods in OpenCLDriver.c. The driver reads ARGB_8888 bitmap pixels via AndroidBitmap_lockPixels, copies raw pixel data into a staging buffer, then initializes the OpenCL runtime (clGetPlatformIDs → clGetDeviceIDs → clCreateContext → clCreateCommandQueue). It compiles the appropriate .cl kernel file at runtime (clCreateProgramWithSource + clBuildProgram), enqueues the NDRange kernel over all pixels, reads results back, releases all CL resources, and returns the processed Bitmap to Java for display. Three separate .cl files cover Blur, Grayscale, and Rotate operations, sharing the same driver flow with only the kernel name and file path differing.",
    "detail_opencl_pipeline_heading": "Implementation Pipeline",
    "detail_opencl_pipeline_step1": "Set up Android project with CMake 3.22, configure AndroidManifest.xml for CAMERA permission, link OpenCL and jnigraphics libraries via target_link_libraries in CMakeLists.txt.",
    "detail_opencl_pipeline_step2": "Implement OpenCLDriver.c: read bitmap metadata (width, height, stride, RGBA format check), copy pixel data to a native staging buffer, and initialize the full OpenCL runtime context and command queue.",
    "detail_opencl_pipeline_step3": "Load and compile .cl kernel files at runtime (clCreateProgramWithSource → clBuildProgram), set kernel arguments for source buffer, destination buffer, image width, and height, then dispatch clEnqueueNDRangeKernel with a local work-group size of 64.",
    "detail_opencl_pipeline_step4": "Read processed pixels back to the native bitmap buffer (clEnqueueReadBuffer), release all OpenCL objects (mem, program, kernel, queue, context), unlock the Android bitmap, and return the result to Java.",
    "detail_opencl_validation_heading": "Performance Results",
    "detail_opencl_validation_item1": "Gaussian blur (CPU sequential): 2059 ms — each pixel requires a weighted average over a 5×5 neighborhood, giving O(W×H×25) scalar operations with no parallelism.",
    "detail_opencl_validation_item2": "Gaussian blur (GPU via OpenCL): 246 ms — identical kernel logic distributed across all pixels in parallel; 8.4× speedup with no algorithmic change, demonstrating the advantage of data-parallel GPU execution.",
    "detail_opencl_validation_item3": "Grayscale (GPU): 81 ms; Rotation (GPU): 72 ms — both pixel-independent transforms run at sub-100 ms on a mobile GPU, confirming that the OpenCL pipeline overhead is amortized quickly for embarrassingly parallel workloads.",
    "detail_opencl_debug_heading": "Debugging Case Study",
    "detail_opencl_debug_symptom_label": "Symptom",
    "detail_opencl_debug_symptom": "The application crashed immediately on launch when the camera preview feature was active on the target Android board, despite permissions being correctly declared in AndroidManifest.xml.",
    "detail_opencl_debug_hypothesis_label": "Hypotheses",
    "detail_opencl_debug_hypothesis": "(1) Runtime permission request not properly handled for the target OS version; (2) Camera API incompatibility between hardware board and the Android version running on it; (3) SurfaceView lifecycle mismatch causing a null camera handle.",
    "detail_opencl_debug_tests_label": "Tests",
    "detail_opencl_debug_tests": "Isolated the camera module into a separate test project — it functioned correctly in isolation, ruling out the permission hypothesis. Cross-referenced the board's Android version against the Camera API deprecation timeline and confirmed a known incompatibility with the specific hardware.",
    "detail_opencl_debug_fix_label": "Fix",
    "detail_opencl_debug_fix": "Removed the camera capture flow from the main project and replaced it with BitmapFactory.decodeFile loading a pre-stored image, preserving the full OpenCL acceleration pipeline intact.",
    "detail_opencl_debug_takeaway_label": "Takeaway",
    "detail_opencl_debug_takeaway": "Hardware-software compatibility on embedded Android requires explicit version gating; when a peripheral subsystem is unstable, decoupling it from the core compute pipeline allows the GPU acceleration work to be validated independently.",
    "detail_opencl_skills_heading": "Skills Demonstrated",
    "detail_opencl_skills_item1": "Low-level GPGPU programming with OpenCL 1.2 C API: context management, kernel compilation, device-memory buffer I/O, and NDRange dispatch on a mobile GPU.",
    "detail_opencl_skills_item2": "Android NDK / JNI integration: authored CMakeLists.txt, linked platform-specific .so libraries, and bridged Java ↔ C across four native functions with bitmap pixel-level access.",
    "detail_opencl_skills_item3": "Performance measurement and analysis: instrumented CPU vs. GPU execution paths with System.nanoTime(), quantified 8.4× speedup, and interpreted results in terms of data-parallel workload characteristics.",
    "detail_opencl_skills_item4": "Systematic debugging on a resource-constrained embedded platform: isolated root cause of hardware API incompatibility, adapted system architecture without compromising the core deliverable.",
    "detail_opencl_artifacts_heading": "Artifacts",
    "detail_opencl_artifact_report": "Project report (PDF)",
    "detail_opencl_artifact_slides": "Presentation slides (PDF)",
    "work_can_title": "CAN Bus Security Analysis and Intrusion Detection via LabVIEW",
    "work_can_sub": "Rate-based IDS with 64-bit multi-signal bit-packing in LabVIEW",
    "work_can_date": "2025.01",
    "detail_can_title": "CAN Bus Security Analysis and Intrusion Detection via LabVIEW",
    "detail_can_subtitle": "Bit-level multi-signal packing with rate-based anomaly detection on a simulated ECU network",
    "detail_can_overview": "CAN (Controller Area Network), developed by Bosch in 1986, is the de-facto serial bus standard for inter-ECU communication in modern vehicles. However, CAN was not designed with security in mind, leaving it vulnerable to spoofing, sniffing, and message injection attacks. This project — conducted as a team of six with myself as team lead — analyzed the full CAN data frame structure and implemented a simulated CAN communication environment in LabVIEW, including a rate-based intrusion detection system. As a creative differentiator, we mapped six human biometric signals (liver function, hemoglobin, brain usage, hydration, gender, stress) into the 64-bit CAN data payload instead of conventional ECU signals, which made the bit-packing architecture both intuitive and demonstrably rigorous. The project won an Excellence Award at the final course presentation.",
    "detail_can_point1": "Led the full project as team lead — designed the 6-signal bit map independently, manually calculating bit boundaries to pack all signals within 64 bits with zero overflow.",
    "detail_can_point2": "Implemented the core LabVIEW Tx/Rx logic using bitwise AND/OR/SHIFT operations inside a Formula Node, along with a dual-version system supporting both integer (U16/U32) and floating-point (DBL) data types.",
    "detail_can_point3": "Architected the rate-based intrusion detection mechanism: a Waveform Chart monitors live packet intervals and triggers an LED alert plus automatic loop termination within a single cycle when anomalous injection is detected.",
    "detail_can_arch_heading": "CAN Frame Structure",
    "detail_can_arch_body": "A standard CAN data frame consists of: SOF (1-bit dominant start), Arbitration Field (11-bit ID for priority arbitration + 1-bit RTR), Control Field (DLC: 4-bit data length code), Data Field (0–64 bits of payload), CRC Field (15-bit CRC-15 sequence for error detection), ACK Field (receiver acknowledgment), and EOF (7 recessive bits). This project used Identifier=5 and DLC=8 (full 64-bit payload). Because CAN carries no source authentication, any node on the bus can inject any message ID — the structural root cause of all three attack vectors analyzed.",
    "detail_can_pack_heading": "Signal Bit-Packing Design",
    "detail_can_pack_body": "Six biometric signals were packed into the 64-bit CAN payload without overlap: LFT (liver, bits 0–9, 10 bits), HEM (hemoglobin, bits 10–28, 19 bits), BRAIN (brain usage, bits 29–49, 21 bits), WATER (hydration, bits 50–52, 3 bits), GENDER (bit 53, 1 bit), STRESS (bits 54–63, 10 bits) — totaling exactly 64 bits. The Tx Formula Node packs these using bitwise masking and shifts (e.g., byte1 = ((LFT & 0x300) >> 8) | ((HEM & 0x3F) << 2)), while the Rx Formula Node applies the inverse to reconstruct all six values. Byte-level integrity was verified by confirming that all-max-value inputs produce byte0–byte7 = 0xFF, and mid-range BRAIN=100000 produces the expected bit patterns across byte3–byte6.",
    "detail_can_vuln_heading": "Security Vulnerabilities & Intrusion Detection",
    "detail_can_vuln_step1": "Lack of Authentication: CAN frames carry no sender credential. Any node with bus access can spoof any message ID, enabling spoofing and replay attacks. This is the foundational vulnerability of the protocol.",
    "detail_can_vuln_step2": "Broadcast Architecture: All nodes simultaneously receive every frame on the bus. There is no unicast — physical access (e.g., via OBD-II port) exposes the entire communication to passive sniffing or active frame injection.",
    "detail_can_vuln_step3": "Rate-Based IDS: Message injection attacks manifest as abnormal packet frequency. The LabVIEW Rx loop measures actual inter-packet intervals and compares them to a configured threshold. When the measured rate exceeds the threshold (e.g., 300 ms vs. 200 ms set value), a hazard LED switches from green to red and the Rx While Loop auto-terminates within one cycle — demonstrated in a live scenario during the course presentation.",
    "detail_can_ext_heading": "Extended Feature: Floating-Point & Negative Value Support",
    "detail_can_ext_body": "Beyond the baseline integer implementation, a second version extended all signal inputs to DBL (64-bit double precision). To pack floating-point values into the integer bit structure, each value is multiplied by 100 before packing and divided by 100 upon reception. Negative values (e.g., stress index = -100) are handled via offset encoding: +100 is added before packing and subtracted after unpacking, allowing the unsigned bit field to represent a signed range without additional bit overhead. This extension demonstrated practical awareness of data type constraints in real-world sensor payloads.",
    "detail_can_skills_heading": "Skills Demonstrated",
    "detail_can_skills_item1": "CAN protocol internals — frame field analysis (SOF/Arbitration/DLC/CRC/ACK/EOF), multi-master arbitration, and ECU bus topology",
    "detail_can_skills_item2": "Low-level bit manipulation — bitwise AND/OR/SHIFT operations for multi-signal packing and unpacking across byte boundaries in LabVIEW Formula Node",
    "detail_can_skills_item3": "Intrusion detection systems — rate-based anomaly detection design, real-time Waveform Chart monitoring, and automatic safety shutdown logic",
    "detail_can_skills_item4": "LabVIEW (NI) — G-language dataflow programming, Front Panel UI design, While Loop / Build Array / Index Array, and integer-to-float type system extension",
    "work_rowscope_title": "RowScope: DRAM Row Buffer Locality Analyzer",
    "work_rowscope_sub": "Quantifying memory access pattern effects on DRAM row buffer performance",
    "work_rowscope_date": "2025.12",
    ...
    "detail_rowscope_overview": "Modern DRAM performance is fundamentally shaped by how software access patterns interact with DRAM bank and row buffer microarchitecture — not just raw bandwidth saturation. RowScope is a complete analytical pipeline that quantifies this relationship: given a known access pattern, what fraction of memory accesses benefit from an already-open DRAM row? The project addresses a gap between high-level performance profiling and low-level DRAM behavior by providing reproducible, simulation-based measurements across sequential, random, stride, and working-set access patterns.",
    "detail_rowscope_point1": "Implemented a five-stage pipeline (C benchmarks → address traces → DRAM address mapper → per-bank row buffer state machine → results aggregator) that classifies every memory access as a row hit, miss, or conflict.",
    "detail_rowscope_point2": "Derived and validated a closed-form hit-rate formula (hit_rate ≈ 1 − stride × 4 / 8192) that accurately predicts degradation across 11 stride values from stride-1 to stride-1024.",
    "detail_rowscope_point3": "Demonstrated a 250× locality gap between sequential access (99.95% hit rate) and random access (0.4% hit rate), and confirmed that sequential locality is invariant to array size across nine log-spaced working sets from 512 KB to 128 MB.",
    "detail_rowscope_pipeline_heading": "Five-Stage Analysis Pipeline",
    "detail_rowscope_pipeline_step1": "C benchmarks (sequential_access.c, random_access.c, stride_access.c, working_set_sweep.c) execute memory access patterns and write virtual addresses to .trace files with metadata headers.",
    "detail_rowscope_pipeline_step2": "The DRAM address mapper (dram_mapping.py) decomposes each address into (bank_id, row_id, col_offset) using configurable row size (8 KB default) and bank count (8 default) via bit-interleaved scheme.",
    "detail_rowscope_pipeline_step3": "A per-bank row buffer state machine (row_buffer_model.py) tracks OPEN/EMPTY state per bank and classifies each access as Row Hit, Row Miss, or Row Conflict in O(1) time.",
    "detail_rowscope_pipeline_step4": "Results are aggregated into summary.csv with hit rate, miss rate, conflict rate, and locality score per benchmark, then six publication-quality figures are generated with Matplotlib.",
    "detail_rowscope_pipeline_step5": "A final report (report/final_report.md) is auto-populated from templates with tables and figures, making the entire experiment reproducible from a single shell script.",
    "detail_rowscope_results_heading": "Key Experimental Results",
    "detail_rowscope_results_body": "Sequential access achieves 99.95% row hit rate — near-perfect locality. Random access (16 MB array) yields only 0.4% hit rate with 99.6% conflict rate. Stride analysis confirms monotonic degradation: stride-256 gives 87.49% hit rate, stride-1024 hits 49.98% (every other access crosses a row boundary), and stride-2048 approaches 0% (one full 8 KB row per step). Working-set sweeps from 512 KB to 128 MB show that sequential hit rate stays constant at 99.95% regardless of array size, confirming that access pattern dominates over working set size.",
    "detail_rowscope_design_heading": "Design Decisions",
    "detail_rowscope_design_item1": "User-space simulation over hardware PMU counters: hardware performance counters (Intel UNC_M_CAS_COUNT) require elevated privilege and vary by CPU generation, making them unavailable in many environments. Simulation gives full control over DRAM geometry parameters for systematic exploration.",
    "detail_rowscope_design_item2": "Virtual address proxy for physical addresses: OS page allocation is non-deterministic for large arrays, but treating malloc virtual addresses as physical addresses is justified because all workloads are analyzed under the same model — systematic errors cancel in comparative studies.",
    "detail_rowscope_design_item3": "Open-page policy only: the most common DDR4 DRAM policy (current row stays open until a conflict) produces the most informative three-way classification (hit / miss / conflict). Closed-page and adaptive policies are left for future work.",
    "detail_rowscope_skills_heading": "Skills Demonstrated",
    "detail_rowscope_skills_item1": "DRAM microarchitecture — row buffer mechanics, bank interleaving, address decomposition, and open-page policy modeling.",
    "detail_rowscope_skills_item2": "Systems measurement methodology — controlled experiment design, virtual address trace analysis, and comparative benchmarking with honest limitation acknowledgment.",
    "detail_rowscope_skills_item3": "Low-level C programming — C99 benchmark authorship with configurable stride, array size, and RNG seed for reproducible trace generation.",
    "detail_rowscope_skills_item4": "Python analysis pipeline — modular data pipeline with DRAMMapper, RowBufferModel, and Matplotlib-based visualization producing publication-quality figures.",
  ...
  "kr": {
    ...
    "detail_5g_artifact_configs": "Bring-up configs & scripts",
    "detail_5g_artifact_logs": "Debug logs & gNB PCAPs",
    "work_opencl_title": "OpenCL 기반 온디바이스 GPU 이미지 처리",
    "work_opencl_sub": "안드로이드 GPU 가속, 8.4배 속도 향상",
    "work_opencl_date": "2023.12",
    "detail_opencl_title": "OpenCL & Android NDK 기반 온디바이스 GPU 이미지 처리",
    "detail_opencl_subtitle": "JNI 브릿지 GPGPU 커널로 가우시안 블러 8.4배 가속",
    "detail_opencl_role": "단독 개발",
    "detail_opencl_timeline": "2023.09 – 2023.12",
    "detail_opencl_stack": "Android (Java), OpenCL 1.2, C (NDK/JNI), CMake 3.22",
    "detail_opencl_overview": "안드로이드 기기의 카메라로 촬영한 이미지에 가우시안 블러, 그레이스케일 변환, 회전 등 실시간 이미지 처리를 적용하는 앱을 개발했습니다. JNI로 연결된 네이티브 라이브러리를 통해 OpenCL 커널을 모바일 GPU에 디스패치하는 방식으로 구현했으며, 마이크로프로세서 응용실습 수업의 최종 프로젝트로서 공개 문서가 부족한 임베디드 환경에서의 GPGPU 프로그래밍을 직접 경험했습니다.",
    "detail_opencl_point1": "OpenCL 1.2 C API를 이용해 전체 GPGPU 파이프라인을 직접 구현했습니다. 플랫폼/디바이스 열거, 컨텍스트 및 커맨드 큐 생성, 런타임 커널 컴파일, 버퍼 I/O(clCreateBuffer / clEnqueueWriteBuffer / clEnqueueReadBuffer), 명시적 자원 해제까지 Android NDK 환경에서 처음부터 작성했습니다.",
    "detail_opencl_point2": "가우시안 블러에서 GPU로 픽셀 병렬 연산을 오프로드하여 CPU 대비 8.4배 속도 향상을 달성했습니다(CPU: 2059ms → GPU: 246ms). 추가로 구현한 그레이스케일(81ms)과 회전(72ms) 커널을 통해 다양한 워크로드에서 GPU 처리량이 일정하게 유지됨을 확인했습니다.",
    "detail_opencl_point3": "CMake + NDK 빌드 파이프라인을 구성하여 libGLES_mali.so와 jnigraphics를 링크하고, OpenCLDriver.c를 공유 라이브러리로 컴파일했습니다. Java 액티비티에서 호출 가능한 JNI 네이티브 함수 4개(GaussianBlurBitmap, GaussianBlurGPU, GaussianGreyGPU, GaussianRotateGPU)를 노출했습니다.",
    "detail_opencl_arch_heading": "시스템 아키텍처",
    "detail_opencl_arch_body": "안드로이드 Java 액티비티(UI + 버튼 이벤트)가 OpenCLDriver.c의 JNI 네이티브 함수를 호출합니다. 드라이버는 AndroidBitmap_lockPixels로 ARGB_8888 비트맵 픽셀을 읽어 스테이징 버퍼에 복사한 뒤 OpenCL 런타임(clGetPlatformIDs → clGetDeviceIDs → clCreateContext → clCreateCommandQueue)을 초기화합니다. 런타임에 .cl 커널 파일을 컴파일(clCreateProgramWithSource + clBuildProgram)하고 전체 픽셀에 대해 NDRange 커널을 디스패치한 후 결과를 읽어 Java로 반환합니다. Blur, Grayscale, Rotate 세 가지 .cl 파일이 동일한 드라이버 흐름을 공유하며 커널 이름과 파일 경로만 다릅니다.",
    "detail_opencl_pipeline_heading": "구현 파이프라인",
    "detail_opencl_pipeline_step1": "CMake 3.22로 Android 프로젝트를 설정하고, AndroidManifest.xml에 CAMERA 권한을 추가하며, CMakeLists.txt의 target_link_libraries로 OpenCL과 jnigraphics 라이브러리를 링크합니다.",
    "detail_opencl_pipeline_step2": "OpenCLDriver.c 구현: 비트맵 메타데이터(너비, 높이, 스트라이드, RGBA 포맷 확인)를 읽고, 픽셀 데이터를 네이티브 스테이징 버퍼에 복사한 후 전체 OpenCL 런타임 컨텍스트와 커맨드 큐를 초기화합니다.",
    "detail_opencl_pipeline_step3": "런타임에 .cl 커널 파일을 로드·컴파일(clCreateProgramWithSource → clBuildProgram)하고, 소스/대상 버퍼, 이미지 너비·높이를 커널 인수로 설정한 뒤 로컬 워크그룹 크기 64로 clEnqueueNDRangeKernel을 디스패치합니다.",
    "detail_opencl_pipeline_step4": "clEnqueueReadBuffer로 처리된 픽셀을 네이티브 비트맵 버퍼에 읽어오고, 모든 OpenCL 객체(mem, program, kernel, queue, context)를 해제한 후 Android 비트맵의 잠금을 해제하고 Java에 결과를 반환합니다.",
    "detail_opencl_validation_heading": "성능 측정 결과",
    "detail_opencl_validation_item1": "가우시안 블러 (CPU 순차 처리): 2059ms — 각 픽셀마다 5×5 이웃 픽셀의 가중 평균 계산이 필요해 O(W×H×25) 스칼라 연산이 발생하며 병렬화 없음.",
    "detail_opencl_validation_item2": "가우시안 블러 (GPU via OpenCL): 246ms — 동일한 커널 로직을 모든 픽셀에 병렬 실행하여 알고리즘 변경 없이 8.4배 속도 향상 달성. 데이터 병렬 GPU 실행의 효과를 명확히 입증.",
    "detail_opencl_validation_item3": "그레이스케일 (GPU): 81ms, 회전 (GPU): 72ms — 픽셀 독립 변환이 모바일 GPU에서 100ms 이하로 실행되어, 병렬 워크로드에서 OpenCL 파이프라인 오버헤드가 빠르게 상쇄됨을 확인.",
    "detail_opencl_debug_heading": "디버깅 케이스 스터디",
    "detail_opencl_debug_symptom_label": "증상",
    "detail_opencl_debug_symptom": "AndroidManifest.xml에 권한이 올바르게 선언되어 있음에도, 카메라 프리뷰 기능이 활성화된 상태에서 앱이 실행 즉시 종료되는 현상이 반복됨.",
    "detail_opencl_debug_hypothesis_label": "가설",
    "detail_opencl_debug_hypothesis": "(1) 대상 OS 버전에서 런타임 권한 요청이 제대로 처리되지 않음; (2) 하드웨어 보드와 Android 버전 간 Camera API 비호환; (3) SurfaceView 생명주기 불일치로 인한 null 카메라 핸들.",
    "detail_opencl_debug_tests_label": "검증",
    "detail_opencl_debug_tests": "카메라 모듈을 별도 테스트 프로젝트로 분리하여 실행 — 단독 환경에서는 정상 동작하여 권한 가설을 배제. 보드의 Android 버전과 Camera API 지원 중단 타임라인을 대조하여 해당 하드웨어와의 알려진 비호환성 확인.",
    "detail_opencl_debug_fix_label": "해결",
    "detail_opencl_debug_fix": "메인 프로젝트에서 카메라 캡처 플로우를 제거하고 BitmapFactory.decodeFile로 저장된 이미지를 로드하는 방식으로 대체. 전체 OpenCL 가속 파이프라인은 그대로 유지.",
    "detail_opencl_debug_takeaway_label": "교훈",
    "detail_opencl_debug_takeaway": "임베디드 Android에서는 하드웨어-소프트웨어 호환성을 명시적으로 버전 게이팅해야 하며, 주변 장치 서브시스템이 불안정할 때 핵심 연산 파이프라인과 분리하면 GPU 가속 작업을 독립적으로 검증할 수 있습니다.",
    "detail_opencl_skills_heading": "입증된 역량",
    "detail_opencl_skills_item1": "OpenCL 1.2 C API를 활용한 저수준 GPGPU 프로그래밍: 컨텍스트 관리, 커널 컴파일, 디바이스 메모리 버퍼 I/O, 모바일 GPU에서의 NDRange 디스패치.",
    "detail_opencl_skills_item2": "Android NDK / JNI 통합: CMakeLists.txt 작성, 플랫폼별 .so 라이브러리 링크, 네이티브 함수 4개에 걸친 Java ↔ C 브리지 및 비트맵 픽셀 수준 접근.",
    "detail_opencl_skills_item3": "성능 측정 및 분석: System.nanoTime()으로 CPU/GPU 실행 경로를 계측하고 8.4배 속도 향상을 정량화하여 데이터 병렬 워크로드 특성으로 해석.",
    "detail_opencl_skills_item4": "임베디드 플랫폼에서의 체계적 디버깅: 하드웨어 API 비호환 근본 원인을 격리하고 핵심 산출물을 유지하면서 시스템 아키텍처를 적응.",
    "detail_opencl_artifacts_heading": "산출물",
    "detail_opencl_artifact_report": "프로젝트 결과 보고서 (PDF)",
    "detail_opencl_artifact_slides": "발표 슬라이드 (PDF)",
    "work_can_title": "CAN Bus Security Analysis and Intrusion Detection via LabVIEW",
    "work_can_sub": "Rate-based IDS with 64-bit multi-signal bit-packing in LabVIEW",
    "work_can_date": "2025.01",
    "detail_can_title": "CAN Bus Security Analysis and Intrusion Detection via LabVIEW",
  ...
  "jp": {
    ...
    "detail_5g_artifact_configs": "Bring-up configs & scripts",
    "detail_5g_artifact_logs": "Debug logs & gNB PCAPs",
    "work_opencl_title": "OpenCLによるオンデバイスGPU画像処理",
    "work_opencl_sub": "Android GPU 加速、8.4倍の高速化",
    "work_opencl_date": "2023.12",
    "detail_opencl_title": "OpenCL & Android NDK によるオンデバイスGPU画像処理",
    "detail_opencl_subtitle": "JNIブリッジGPGPUカーネルでガウシアンブラーを8.4倍高速化",
    "detail_opencl_role": "単独開発者",
    "detail_opencl_timeline": "2023.09 – 2023.12",
    "detail_opencl_stack": "Android (Java), OpenCL 1.2, C (NDK/JNI), CMake 3.22",
    "detail_opencl_overview": "Androidデバイスのカメラで撮影した画像に、JNIブリッジ経由のネイティブライブラリからOpenCLカーネルをモバイルGPUにディスパッチして、ガウシアンブラー・グレースケール変換・回転などのリアルタイム画像処理を行うアプリを開発しました。マイクロプロセッサ応用実習の最終課題として、公開ドキュメントが乏しいリソース制約の組み込みプラットフォームでGPGPUプログラミングを実践的に体験しました。",
    "detail_opencl_point1": "OpenCL 1.2 C APIを用いてフルGPGPUパイプラインをゼロから実装：プラットフォーム・デバイス列挙、コンテキスト・コマンドキュー作成、実行時カーネルコンパイル、バッファI/O（clCreateBuffer / clEnqueueWriteBuffer / clEnqueueReadBuffer）、明示的リソース解放をAndroid NDK環境で一から構築。",
    "detail_opencl_point2": "ガウシアンブラーでGPUへのピクセル並列オフロードにより8.4倍の高速化を達成（CPU: 2059ms → GPU: 246ms）。さらにグレースケール（81ms）と回転（72ms）カーネルも実装し、異種ワークロードでも安定したGPUスループットを確認。",
    "detail_opencl_point3": "CMake + NDKビルドパイプラインを設計し、libGLES_mali.soとjnigraphicsをリンク、OpenCLDriver.cを共有ライブラリとしてコンパイル。JavaアクティビティからJNI経由で呼び出せる4つのネイティブメソッド（GaussianBlurBitmap, GaussianBlurGPU, GaussianGreyGPU, GaussianRotateGPU）を公開。",
    "detail_opencl_arch_heading": "システムアーキテクチャ",
    "detail_opencl_arch_body": "Android JavaアクティビティのボタンイベントがOpenCLDriver.cのJNIネイティブメソッドを呼び出します。ドライバはAndroidBitmap_lockPixelsでARGB_8888ビットマップのピクセルを読み取り、ステージングバッファにコピーした後、OpenCLランタイム（clGetPlatformIDs → clGetDeviceIDs → clCreateContext → clCreateCommandQueue）を初期化。実行時に.clカーネルファイルをコンパイル（clCreateProgramWithSource + clBuildProgram）し、全ピクセルにNDRangeカーネルをディスパッチ、結果を読み戻してJavaに返します。Blur・Grayscale・Rotateの3つの.clファイルが同一のドライバフローを共有し、カーネル名とファイルパスのみ異なります。",
    "detail_opencl_pipeline_heading": "実装パイプライン",
    "detail_opencl_pipeline_step1": "CMake 3.22でAndroidプロジェクトをセットアップし、AndroidManifest.xmlにCAMERAパーミッションを追加、CMakeLists.txtのtarget_link_librariesでOpenCLとjnigraphicsをリンク。",
    "detail_opencl_pipeline_step2": "OpenCLDriver.cを実装：ビットマップのメタデータ（幅・高さ・ストライド・RGBAフォーマット確認）を読み取り、ピクセルデータをネイティブステージングバッファにコピーし、OpenCLランタイムのコンテキストとコマンドキューを初期化。",
    "detail_opencl_pipeline_step3": "実行時に.clカーネルファイルをロード・コンパイル（clCreateProgramWithSource → clBuildProgram）し、ソース/デスティネーションバッファと画像の幅・高さをカーネル引数に設定、ローカルワークグループサイズ64でclEnqueueNDRangeKernelをディスパッチ。",
    "detail_opencl_pipeline_step4": "clEnqueueReadBufferで処理済みピクセルをネイティブビットマップバッファに読み戻し、全OpenCLオブジェクト（mem, program, kernel, queue, context）を解放してAndroidビットマップのロックを解除し、結果をJavaに返す。",
    "detail_opencl_validation_heading": "パフォーマンス測定結果",
    "detail_opencl_validation_item1": "ガウシアンブラー（CPUシーケンシャル）: 2059ms — 各ピクセルに5×5近傍の加重平均計算が必要で、O(W×H×25)のスカラー演算が発生し並列化なし。",
    "detail_opencl_validation_item2": "ガウシアンブラー（GPU via OpenCL）: 246ms — 同一カーネルロジックを全ピクセルに並列実行し、アルゴリズム変更なしで8.4倍高速化を達成。データ並列GPU実行の優位性を明確に実証。",
    "detail_opencl_validation_item3": "グレースケール（GPU）: 81ms、回転（GPU）: 72ms — ピクセル独立変換がモバイルGPUで100ms未満で実行され、OpenCLパイプラインのオーバーヘッドが並列ワークロードで素早く償却されることを確認。",
    "detail_opencl_debug_heading": "デバッグケーススタディ",
    "detail_opencl_debug_symptom_label": "症状",
    "detail_opencl_debug_symptom": "AndroidManifest.xmlに正しくパーミッションが宣言されているにもかかわらず、カメラプレビュー機能が有効な状態でアプリが起動直後にクラッシュする現象が繰り返し発生。",
    "detail_opencl_debug_hypothesis_label": "仮説",
    "detail_opencl_debug_hypothesis": "(1) 対象OSバージョンでのランタイムパーミッション処理の不備; (2) ハードウェアボードとAndroidバージョン間のCamera API非互換; (3) SurfaceViewライフサイクルの不一致によるnullカメラハンドル。",
    "detail_opencl_debug_tests_label": "検証",
    "detail_opencl_debug_tests": "カメラモジュールを別テストプロジェクトに分離して実行 — 単独では正常動作し、パーミッション仮説を除外。ボードのAndroidバージョンとCamera API非推奨タイムラインを照合し、特定ハードウェアとの既知の非互換性を確認。",
    "detail_opencl_debug_fix_label": "修正",
    "detail_opencl_debug_fix": "メインプロジェクトからカメラキャプチャフローを削除し、BitmapFactory.decodeFileで保存済み画像を読み込む方式に変更。OpenCL加速パイプライン全体はそのまま維持。",
    "detail_opencl_debug_takeaway_label": "教訓",
    "detail_opencl_debug_takeaway": "組み込みAndroidではハードウェアとソフトウェアの互換性を明示的にバージョンゲーティングする必要があり、周辺サブシステムが不安定な場合はコアパイプラインから分離することでGPU加速部分を独立して検証できます。",
    "detail_opencl_skills_heading": "実証したスキル",
    "detail_opencl_skills_item1": "OpenCL 1.2 C APIによる低レベルGPGPUプログラミング：コンテキスト管理、カーネルコンパイル、デバイスメモリバッファI/O、モバイルGPUでのNDRangeディスパッチ。",
    "detail_opencl_skills_item2": "Android NDK / JNI統合：CMakeLists.txtの作成、プラットフォーム固有.soライブラリのリンク、4つのネイティブ関数でのJava ↔ Cブリッジおよびビットマップピクセルレベルアクセス。",
    "detail_opencl_skills_item3": "パフォーマンス計測と分析：System.nanoTime()でCPU/GPU実行パスを計測し、8.4倍高速化を定量化してデータ並列ワークロードの特性で解釈。",
    "detail_opencl_skills_item4": "リソース制約された組み込みプラットフォームでの体系的デバッグ：ハードウェアAPI非互換の根本原因を分離し、コア成果物を維持しながらシステムアーキテクチャを適応。",
    "detail_opencl_artifacts_heading": "成果物",
    "detail_opencl_artifact_report": "プロジェクト結果報告書（PDF）",
    "detail_opencl_artifact_slides": "発表スライド（PDF）",
    "work_can_title": "CAN Bus Security Analysis and Intrusion Detection via LabVIEW",
    "work_can_sub": "Rate-based IDS with 64-bit multi-signal bit-packing in LabVIEW",
    "work_can_date": "2025.01",
    "detail_can_title": "CAN Bus Security Analysis and Intrusion Detection via LabVIEW",
